...
const AIChat = () => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.data?.data?.chat) {
        ...
        setChats((prev) =>
          prev.map((c) =>
            c._id === tempUserMessage._id ? res.data.data.chat : c,
          ),
        );
      }
    } catch (err) {
      setError(
        err.response?.data?.message ||
          "AI assistant request failed. Rate limit exceeded?",
      );
  ...
  const handleApplyTemplate = (type, topicLabel) => {
    ...
    if (type === "explain") {
      ...
      setPrompt(
        `Generate a 3-question multiple choice quiz on Javascript arrays.`,
      );
  ...
  const formatText = (text) => {
    ...
    let formatted = text
      ...
      .replace(
        /`(.*?)`/g,
        '<code style="background: rgba(255, 255, 255, 0.1); padding: 2px 6px; border-radius: 4px; font-family: monospace; color: #a855f7;">$1</code>',
      )
      .replace(/\n/g, "<br />");
    return (
      <div
        ...
        >
          {historyLoading ? (
            ...
            >
              ...
              >
                Ask your study buddy to explain concepts, generate quizzes, or
                summarize complex notes. Choose a quick template to start:
              </p>
              ...
              >
                <button
                  onClick={() => handleApplyTemplate("quiz", "Coding")}
                ...
                >
                  <Code size={16} color="var(--primary)" /> Generate Javascript
                  Quiz
                </button>
                <button
                  onClick={() => handleApplyTemplate("explain", "Science")}
                  className="btn-secondary"
                  style={{
                    fontSize: "0.85rem",
                    width: "100%",
                ...
                >
                  <Brain size={16} color="var(--secondary)" /> Explain Quantum
                  Computing
                </button>
                <button
                  onClick={() => handleApplyTemplate("summarize", "History")}
                  className="btn-secondary"
                  style={{
                    fontSize: "0.85rem",
                    width: "100%",
                ...
                >
                  <FileText size={16} color="var(--accent)" /> Summarize World
                  War I History
                </button>
              </div>
            </div>
          ...
          {loading && (
            ...
            >
              <span style={{ fontSize: "0.85rem", color: "var(--text-muted)" }}>
                AI Study Buddy is drafting response...
              </span>
            </div>
          )}
